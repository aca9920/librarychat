...
let response = "";
let graphicTitle;
let graphicBookBackground;

function preload() {
  bot.loadFile("graphic.txt").then(loaded).catch(error);

  graphicTitle = loadImage("graphicTitle.png");
  graphicBookBackground = loadImage("graphicBookBackground.png")
}
function setup() {
...
function draw() {
  ...
  image(graphicTitle, width/2-230, height/2-580, 450,300);
  image(graphicBookBackground, width/2-550, height/2-410, 1090, 1090);
