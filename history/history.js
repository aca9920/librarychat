...
let response = "";
let historyTitle;
let historyBookBackground;

function preload() {
  bot.loadFile("history.txt").then(loaded).catch(error);

  historyTitle = loadImage("historyTitle.png");
  historyBookBackground = loadImage("historyBookBackground.png")
}
function setup() {
...
function draw() {
  ...
  image(historyTitle, width/2-230, height/2-580, 450,300);
  image(historyBookBackground, width/2-550, height/2-410, 1090, 1090);
