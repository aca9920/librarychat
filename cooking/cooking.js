...
let response = "";
let cookingTitle;
let cookingBookBackground;

function preload() {
  bot.loadFile("cooking.txt").then(loaded).catch(error);

  cookingTitle = loadImage("cookingTitle.png");
  cookingBookBackground = loadImage("cookingBookBackground.png")
}
function setup() {
...
function draw() {
  ...
  image(cookingTitle, width/2-230, height/2-580, 450,300);
  image(cookingBookBackground, width/2-550, height/2-410, 1090, 1090);
